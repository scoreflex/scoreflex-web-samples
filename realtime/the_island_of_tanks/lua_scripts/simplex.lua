...
SimplexNoise.Noise2D = function(xin, yin)
   ...
   if t0 < 0 then
      ...
      t0 = t0 * t0
      n0 = t0 * t0 * SimplexNoise.Dot2D(SimplexNoise.Gradients3D[gi0], x0, y0)
   end
...
--very expensive, much more so that standard 2D noise.
SimplexNoise.FractalSum2DNoise = function(x, y, itier)
...
--very expensive, much more so that standard 2D noise.
SimplexNoise.FractalSumAbs2DNoise = function(x, y, itier)
 ...
 --very expensive, much more so that standard 2D noise.
SimplexNoise.Turbulent2DNoise = function(x, y, itier)
   local ret = math.abs(SimplexNoise.Noise2D(x, y))
   for i=1,itier do
      local itier = 2^itier
      ret = ret + (i/itier) * math.abs(SimplexNoise.Noise2D(x * (itier/i), y * (itier/i)))
